...
import { getDoctorBySpecializationSlug } from "../../../../services/homeService";
...
function DetailSpecialization() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            setLoading(true);
            try {
                const result = await getDoctorBySpecializationSlug(slug);
                setDoctors(result.doctors);
                setSpecializationInfo(result.specialization);
            } catch (err) {
                console.error(err);
            } finally {
                setLoading(false);
    ...
    return (
        <div className="container mt-4">
            <div className="specialization-detail mb-4 d-flex justify-content-between flex-wrap">
                <div>
                    <h2>Chuyên khoa {specializationInfo.name}</h2>
                    <p>{specializationInfo.description}</p>
                </div>
            ...
            {loading ? (
                ...
                <p>Không có bác sĩ cho chuyên khoa này.</p>
            ) : (
                doctors.map((doc) => (
                    <DoctorCard
                        key={doc._id}
