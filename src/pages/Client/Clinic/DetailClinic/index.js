...
import { getDoctorByClinicSlug } from "../../../../services/homeService";
import BookingModal from "../../../../components/ScheduleBooking";
...
function DetailClinic() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            setLoading(true);
            try {
                const result = await getDoctorByClinicSlug(slug);
                console.log(result);
    ...
    return (
        <div className="container mt-4">
            <div className="clinic-detail mb-4 d-f">
...
function ClinicInfoCard({ clinicInfo }) {
    return (
        <div
        ...
        >
            ...
            <div style={{ flex: 1 }}>
                <h2 style={{ marginBottom: "10px" }}>Phòng khám {clinicInfo.name}</h2>
                <p style={{ marginBottom: "6px" }}>{clinicInfo.description}</p>
                <p style={{ marginBottom: "6px" }}>Giờ mở cửa: {clinicInfo.openingHours}</p>
                <p style={{ marginBottom: "6px" }}>Số điện thoại: {clinicInfo.phone}</p>
                <p style={{ marginBottom: "6px" }}>Địa chỉ: {clinicInfo.address}</p>
            </div>
        </div>
    );
}
